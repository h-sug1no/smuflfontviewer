...
class SMuFLFontViewer {
  ...
  init(options) {
    ...
    sMuFLMetadata.init(options).then(function (/* obj */) {
      if (sMuFLMetadata.initErrors.length) {
        alert(
          sMuFLMetadata.initErrors.map(function (str) {
            return str + "\n";
          })
        );
      } else {
        ...
        const initCpSelect = ($codepointSelect, onChangeCB, onBlurCB) => {
          ...
          $codepointSelect_selectize.onType = function (str, keepOptions) {
            ...
            if (str.match(/^[A-F0-9]+$/)) {
              ...
              /* const cpData = */ $codepointSelect_selectize.addCodePointItem(
                str
              );
        ...
        const cpSelect0 = initCpSelect(
          ...
          ($codepointSelect_selectize) => {
            if (!$codepointSelect_selectize.getValue().length) {
              $codepointSelect_selectize.setValue(
                history.currentUCodepoint,
                true
              );
            }
          }
    ...
    for (let li = 0; li < hintLabels.length; li++) {
      ...
      const isIndeterminate =
        hintLabel.textContent.startsWith("stem") ||
      ...
      if (
        isIndeterminate ||
        hintLabel.textContent.startsWith("repeatOffset") ||
    ...
    const $smuflGlyphHints_repatOffset3StateBox = $smuflGlyphHints.find(
      "#" + toHintlabelIdStr("repeatOffset") + " input"
    );
    const smuflGlyphHints_repatOffset3StateBoxElm =
      $smuflGlyphHints_repatOffset3StateBox[0];

    const $smuflGlyphHints_cutOutOrigin_BBL = $smuflGlyphHints.find(
      "#" + toHintlabelIdStr("cutOutOrigin_BBL") + " input"
    );
    ...
    const $smuflRenderGlyphOptionsGlyphSize = $(
    ...
    const $smuflRenderGlyphOptionsHideAll = $(
      "#smuflRenderGlyphOptionsHideAll"
    );
    const $smuflGlyphUIContainer = $("#smuflGlyphUIContainer");
    const $smuflRenderGlyphOptionsToggleAll = $(
      "#smuflRenderGlyphOptionsToggleAll"
    );

    $smuflRenderGlyphOptionsToggleAll.on("click", function () {
      const $checkBoxElms = $('#smuflGlyphUIContainer input[type="checkbox"]');
    ...
    const $scratchpadDialogInsertButton = $(
      '#scratchpadDialog .controls input[type="button"]'
    );
    ...
    function _initMouseHandlers() {
      ...
      $smuflGlyphCanvasContainer.on("mousemove", function (ev) {
        if (isActive) {
          $smuflGlyphCanvasContainer.scrollTop(
            startPos.scrollTop - (ev.clientY - startPos.clientY)
          );
          $smuflGlyphCanvasContainer.scrollLeft(
            startPos.scrollLeft - (ev.clientX - startPos.clientX)
          );
        }
      });
    ...
    const _$infoDialog_defaultDescription =
      "use browser's search to find glyphname, smufl properties, codepoint, etc...";
    ...
    if (!infoDialogElm.showModal) {
      ...
      infoDialogElm.showModal = function (
        title,
        description = _$infoDialog_defaultDescription
      ) {
    ...
    function appendCodepoint($c, uCodepointStr, currentUCodepoint) {
      ...
      $c.append(
        $(`<span class="smuflCodepoint${currentGlyph}">${uCodepointStr}</span>`)
      );
    }
    ...
    function appendAlternateCodepointFors($c, uCodepointStr) {
      ...
      glyphs.forEach(function (glyph) {
        ...
        const tUCodepoint = sMuFLMetadata.glyphname2uCodepoint(
          glyph.glyphname,
          option
        );
    ...
    function setCodepointByString(codepointStr) {
      ...
      if (rosgCpSelect && rosgCpSelect.$codepointSelect_selectize) {
        rosgCpSelect.$codepointSelect_selectize.setValue(
          history.currentUCodepoint,
    ...
    function seekToCodepoint(cpNumber, d, checkHasGlyph) {
      ...
      while ((cpNumber += d) >= 0 && (cpNumber < 0x10ffff) & !codepointStr) {
        ...
        if (checkHasGlyph && glyphsByUCodepoint) {
          if (
            glyphsByUCodepoint[sMuFLMetadata.ensureUCodepoint(tCodepointStr)]
          ) {
    ...
    function addGlyphnameInfo($contentContainer, ginfo, glyphname) {
      ...
      if (ginfo.alternateCodepoint) {
        _$c_appendText(
          $contentContainer,
          `, alternateCodepoint: ${ginfo.alternateCodepoint}: `
        );
      }
    ...
    function _$infoDialog_showModal(keyIn, func) {
      ...
      if (!$contentDom) {
        $contentDom = _$infoDialog_contentDoms[key] = $(
          '<div class="infoDialogContents"></div>'
        );
        func($contentDom);
      }
    ...
    mkSpecLinkDom(specLinkDoms, "font metadata optionalGlyphs", [
      "optionalglyphs",
    ]);
    mkSpecLinkDom(specLinkDoms, "font metadata", [
    ...
    mkSpecLinkDom(specLinkDoms, "font metadata glyphsWithAlternates", [
      "glyphswithalternates",
    ]);
    mkSpecLinkDom(specLinkDoms, "font metadata glyphsWithAnchors", [
      "glyphswithanchors",
    ]);

    $("#BGlyphnames").on("click", function () {
      _$infoDialog_showModal(
        specLinkDoms.glyphnames,
    ...
    $("#BOptionalGlyphs").on("click", function () {
      _$infoDialog_showModal(
        specLinkDoms["font metadata optionalGlyphs"],
        function ($contentContainer) {
          const optionalGlyphs = sMuFLMetadata.fontMetadata().optionalGlyphs;
          if (!optionalGlyphs) {
            return;
      ...
      _$infoDialog_showModal(
        ...
        function ($contentContainer) {
          ...
          function add_engravingDefaults(name, engravingDefaults) {
            ...
            const $tdContaienr = $(
              '<div class="engravingDefaultsContainer"></div>'
            );
            for (const key in engravingDefaults) {
              _$c_appendText(
                $tdContaienr,
                `${key}: ${engravingDefaults[key]}, `
              );
            }
          ...
          function add_sets(name, sets) {
            ...
            _$c_appendText(
              $contentContainer,
              `${name}: ${setsKeys.length ? setsKeys.join(", ") : "none"}`
            );
          }

          const urls = sMuFLMetadata.urls;
          const $urlsContainer = $(
            '<div class="fontMetadataUrlsContainer"></div>'
          );
    ...
    $("#BFontMetadataLigatures").on("click", function () {
      _$infoDialog_showModal(
        specLinkDoms["font metadata ligatures"],
        function ($contentContainer) {
          try {
            const ligatures =
              sMuFLMetadata.getFontInfo().fontMetadata.ligatures;
            Object.keys(ligatures).forEach(function (glyphname) {
              const $ligaturesInfo = $(
                `<div class="ligatureContainer glyphContainer"></div>`
              );
    ...
    function _createAnyListPage(
      ...
      function _addLink($c, hrefName, clazz) {
        ...
        if (disabled) {
          ...
          $c.append(
            $(`<a class=${clazz} href="#${_hrefId(hrefName)}">${hrefName}</a> `)
          );
        }
      }
      ...
      try {
        ...
        dictKeys.forEach(function (itemName, idx, items) {
          ...
          const $itemContainer = $(
            `<div class="${listName}Container" id="${id}"></div>`
          );
    ...
    $("#BFontMetadataSets").on("click", function () {
      _$infoDialog_showModal(
        specLinkDoms["font metadata sets"],
        function ($contentContainer) {
          _createAnyListPage(
            $contentContainer,
            "set",
    ...
    $("#BFontMetadataGlyphsWithAlternates").on("click", function () {
      _$infoDialog_showModal(
        specLinkDoms["font metadata glyphsWithAlternates"],
        function ($contentContainer) {
          try {
            const gwAlternates =
              sMuFLMetadata.fontMetadata().glyphsWithAlternates;
            for (const akey in gwAlternates) {
              const alternates = gwAlternates[akey];
              const $gwaContainer = $(
                `<div class="gwalternatesContainer glyphContainer"></div>`
              );
    ...
    $("#BFontMetadataGlyphsWithAnchors").on("click", function () {
      _$infoDialog_showModal(
        specLinkDoms["font metadata glyphsWithAnchors"],
        function ($contentContainer) {
          const btns = [];
          const checkboxes = [];
          ...
          try {
            const $gwaOptionContainer = $(
              `<div class="gwanchorsOptionContainer">filter: </div>`
            );
            $contentContainer.append($gwaOptionContainer);
            $gwaOptionContainer.prop(
              "title",
              "check anchors to show items with a specific anchors"
            );
            ...
            [
              ...
              const sIdx = styleSheet.insertRule(
                `.${aName} {display: block}`,
                styleSheet.cssRules.length
              );
              const $cbLabel =
                $(`<label class="gwanchorOption"><input type="checkbox" name="${aName}"
                checked/>${aName}</label>`);
              const $checkbox = $cbLabel.find("input");
              checkboxes.push({
                $checkbox,
                onChange: () => {
                  styleSheet.cssRules[sIdx].style.display = $checkbox.prop(
                    "checked"
                  )
                    ? "block"
                    : "";
            ...
            const glyphsWithAnchors =
              sMuFLMetadata.getFontInfo().fontMetadata.glyphsWithAnchors;
            Object.keys(glyphsWithAnchors).forEach(function (glyphname) {
              ...
              $gwaContainer.prop(
                "class",
                Object.keys(glyph).concat("gwanchorsContainer").join(" ")
              );
            });
          } catch (e) {
            console.log(e);
    ...
    $("#BCCStaticLink").on("click", function () {
      ...
      _$infoDialog_showModal(key, function ($contentContainer) {
        try {
          ...
          const $urlText = $(
            `<textarea class="urlText" readonly title=""></textarea>`
          );
          $stlinkContainer.append($urlText);
          $contentContainer.$urlText = $urlText;

          const $msgText = $(
            `<div class="msgText">URL copied to clipboard.</div>`
          );
    ...
    function _anchorCsToScreenCs(scaledBBox, anchor, sbl, relativeToBBL) {
      return {
        x:
          ...
          anchorCsToScreenCsX(Number(anchor[0]), sbl),
        y:
          (relativeToBBL ? scaledBBox.S : scaledBBox.y) +
          anchorCsToScreenCsY(Number(anchor[1]), sbl),
      };
    }
    ...
    function _renderStemEx(nhScaledBBox, w, endY, bb, stemAnchorName) {
      ...
      const stemAttachmentPos = _anchorCsToScreenCs(
        nhScaledBBox,
        stemAnchor,
        nhScaledBBox.sbl
      );
      ...
      const x =
        stemAttachmentPos.x -
        w * 0.5 * (stemAnchorName.endsWith("NW") ? -1 : 1);
    ...
    function _renderAllteredUnison(halign, vdir, w, nhScaledBBox, bb) {
      ...
      if (halign === "R") {
        // n ap [nh1 p b ap] nh0
        nh1X -=
          afm.scaledBBox.w + nhScaledBBox.w + (paddingX + accidentalPaddingX);
      } else {
        // b ap [nh0 p n ap nh1]
        nh1X +=
          nhScaledBBox.w + anm.scaledBBox.w + (paddingX + accidentalPaddingX);
      }
    ...
    function _renderStem(
      ...
      if (isSplitStem) {
        ...
        const dw =
          auInfo.stemAttachmentX - x - w * 0.5 * (halign === "L" ? 1 : -1);
    ...
    function _renderGraceNoteSlash(bbs, engravingDefaults, sbl) {
      ...
      slashBbs.forEach(function (sbb) {
        if (
          sbb[0] &&
          sbb[1] &&
          (!sbb[0].isIndeterminate || !sbb[1].isIndeterminate)
        ) {
          ctx.save();
          ctx.lineWidth = w;
    ...
    function renderGlyph(glyphData) {
      ...
      */
      ...
      if (
        _shouldDrawProp($smuflGlyphHints_repatOffset3StateBox.prop("checked"))
      ) {
        ctx.save();
        if (repeatOffset) {
          ...
          _renderGlyph(
            tGlyphData,
            x + anchorCsToScreenCsX(repeatOffset[0], sbl),
      ...
      if (anchor) {
        ...
        for (const akey in anchorDefs) {
          ...
          const $anchorCheckbox = $smuflGlyphHints.find(
            "#" + toHintlabelIdStr(akey) + " input"
          );
          _setValValue($anchorCheckbox, anchor[akey]);
          const isIndeterminate = $anchorCheckbox.prop("indeterminate");
          if (
            (!$anchorCheckbox.prop("checked") && !isIndeterminate) ||
    ...
    function _draw() {
      ...
      glyphnameData =
        glyphnameData ||
        (optionalGlyphs ? optionalGlyphs[glyphname] : undefined);
      ...
      if (!glyphnameData) {
        ...
        for (const key in glyphnameData) {
          ...
          if (key !== "classes") {
            _$c_appendText($smuflGlyphInfoText, key + ": ");
            appendCodepointOrText(
              $smuflGlyphInfoText,
              glyphnameData[key],
              uCodepoint
            );
      ...
      if (tRange) {
        ...
        if (tRange.r.noSpecLink) {
          ...
          const $aDom =
            $(`<a href="https://w3c.github.io/smufl/latest/tables/${filename}.html"
 title="${tRange.key} range spec">${tRange.key}</a>`);
      ...
      if (fontInfo) {
        ...
        [setsByAlternateForItem, setsByNameItem].forEach(function (items) {
          ...
          items.forEach(function (item) {
            ...
            _$c_appendText(
              $setInfo,
              `description: ${item.set.description}, type: ${item.set.type}`
            );
            ...
            _$c_appendText(
              $setInfo,
              `, description: ${item.glyph.description}`
            );
      ...
      if (hasCutOut) {
        $smuflGlyphHints
          .children(`#${toHintlabelIdStr("cutOutOrigin_BBL")}`)
          .show();
      }
    ...
    this._handle_onResourceReady = function (type) {
      ...
      $rangeSelect.selectize({
        ...
        onBlur: function () {
          if (!$rangeSelect_selectize.getValue().length) {
            $rangeSelect_selectize.setValue(
              $rangeSelect_selectize.currentValue_,
              true /*silent: no changed event*/
            );
          }
        },
      ...
      const settings_cutOutOrigin_BBL =
        params.get("settings.cutOutOrigin_BBL") === "true";
      $smuflGlyphHints_cutOutOrigin_BBL.prop(
        "checked",
        settings_cutOutOrigin_BBL
      );
      ...
      $("#BFontMetadataGlyphsWithAnchors").prop(
        "disabled",
        !fontMetadata.glyphsWithAnchors
      );
      $("#BFontMetadataGlyphsWithAlternates").prop(
        "disabled",
